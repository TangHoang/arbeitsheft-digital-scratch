...
const RoehrenNichtGleichzeitigVerschwinden = async function (t) {
  ...
  t.log(`Innerhalb ${MAX_CHECK_STEPS} Steps ist keine Röhre verschwunden – Gleichzeitigkeit damit (im Beobachtungsfenster) ausgeschlossen.`);
};
...
const RoehrenYBewegung = async function (t) {
  ...
  t.assert.ok(
    r1Changed,
    `${R1} hat seine y-Position innerhalb ${MAX_STEPS} Steps nicht groß geändert (Δ<${DELTA_MIN}).`
  );
  t.assert.ok(
    r2Changed,
    `${R2} hat seine y-Position innerhalb ${MAX_STEPS} Steps nicht groß geändert (Δ<${DELTA_MIN}).`
  );
};
...
module.exports = [
  {
    ...
    description: 'Während des Spiels ist der horizontale Abstand ausreichend.',
    categories: []
  },
  {
    test: RoehrenNichtGleichzeitigVerschwinden,
    name: 'Nicht gleichzeitig verschwinden',
    description: 'Wenn eine Röhre verschwindet, ist die andere gleichzeitig noch sichtbar im Bild.',
    categories: []
  },
  {
